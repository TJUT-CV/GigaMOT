...
from yolox.tracker.gigatracker import GigaTracker
...
def show_track_img(img,txt,frame_count,all_frame_count):
    ...
    # exit()
    show_font_num = 2
    cv2.putText(img, '#{}/{}'.format(frame_count, all_frame_count), (20, 50), cv2.FONT_HERSHEY_COMPLEX,
                show_font_num, (0, 255, 255), 3)
    for a in range(len(txt)):
        bbox = np.array(txt[a][2:6] * scale, dtype=int)
        ret = bbox
        bbox = xywh2xyxy(bbox)
        bbox = np.array(bbox, dtype=int)
        p_id = int(txt[a][1])
        cv2.rectangle(img, (bbox[0],bbox[1]),(bbox[2],bbox[3]), genColorByPid(p_id), 3)
        cv2.putText(img, '{}'.format(p_id), (bbox[0], bbox[1]), cv2.FONT_HERSHEY_COMPLEX, 1, genColorByPid(p_id), 1)
    print('1111111111aaaaaaaaaaaaaaaaaaaaaaaaa')
    return img

def show_img(img,bbox,frame_count,all_frame_count):
    imgheight, imgwidth = img.shape[:2]
    showwidth = 1920
    scale = showwidth / imgwidth
    img = cv2.resize(img, (int(imgwidth * scale), int(imgheight * scale)))
...
def show_multi_dete(img,bbox):
    for box in bbox:
        img=cv2.rectangle(img, (int(box[0]),int(box[1])),(int(box[2]),int(box[3])), (0, 0, 255),2)
        # img=cv2.rectangle(img, (int(box[2]),int(box[3])),(int(box[4]),int(box[5])), (0, 0, 255),2)
    return img
def show_multi_track(img,bbox):
    for box in bbox:
        img=cv2.rectangle(img, (int(box[2]),int(box[3])),(int(box[2]+box[4]),int(box[3]+box[5])), genColorByPid(box[1]),2)
    return img
def make_parser():
    ...
    # parser.add_argument("--track_thresh", type=float, default=0.1, help="tracking confidence threshold") ############# 593
    parser.add_argument("--track_thresh", type=float, default=0.4, help="tracking confidence threshold") ############# 593

    parser.add_argument("--track_buffer", type=int, default=16, help="the frames for keep lost tracks") ###################

    parser.add_argument("--match_thresh", type=float, default=1, help="matching threshold for tracking")
    parser.add_argument(
    ...
    #
    # parser.add_argument("--track_thresh", type=float, default=0.40, help="tracking confidence threshold") ############# 593
    #
    # parser.add_argument("--track_buffer", type=int, default=16, help="the frames for keep lost tracks") ###################
    #
    # parser.add_argument("--match_thresh", type=float, default=1, help="matching threshold for tracking")
...
def image_demo(predictor,  current_time, args):
    ...
    frame_id = 1
    results = []
    tracker = GigaTracker(args, frame_rate=int(args.fps))
    if args.show_track:
        print('111111111111222222222333333333333333')
        cv2.namedWindow('tracking',cv2.WINDOW_NORMAL)
        print('11111111111122222222233333333333333333333333eeeeeeeeeeeeeeee')
    if args.show_dete:
        print('1111111111112222222223333333333333334444444444444444444')
        cv2.namedWindow('detection', cv2.WINDOW_NORMAL)
        print('111111111111222222222333333333333333444444444444444444433333333333eeeeeeeeeeeeeee')

    for path, src_giga_img, sub_img_list in dataloader:
        img_info_track = {}
        img_info_track['height'] = src_giga_img.shape[0]
        img_info_track['width'] = src_giga_img.shape[1]
        clone_flag = False
        pred = torch.Tensor()
        tic = time.time()
        tic_det = time.time()
        ...
        temp_results = []
        temp_track_results = []
        # print('000000000000000000000ooooooooooooooooooooo   frame:{}'.format(frame_id))
        # print(outputs)
        if outputs is not None:
            temp_outputs = outputs
            timer2.tic()
            w_num = 3
            outputs_w = np.array(outputs[:, w_num])
            temp_n = 30
            temp_n2 = temp_n + 1
            # online_targets, no_online_targets = tracker.update(outputs,  args=args,img_info=img_info_track)
            # online_targets, no_online_targets = tracker.update(temp_outputs, args=args, img_h=img_info_track['height'],
            #                                                            img_w=img_info_track['width'])
            online_targets, no_online_targets=tracker.update(outputs, img_info, obj=obj, args=args, img=src_giga_img)
            ...
            for t in online_targets:
                ...
                if True:
                    ...
                    temp_track_results.append([frame_id,tid,tlwh[0],tlwh[1],tlwh[2],tlwh[3],-1,-1,-1,-1])

            timer.toc()
            timer2.toc()
            print('4    tracking cost time: {} s'.format(timer2.average_time))
            if args.show_dete:
                img = show_img(src_giga_img, temp_outputs,frame_id,len(dataloader))
                cv2.imshow('detection', img)
                cv2.moveWindow('detection', 20, 20)
                show_time = 1
            ...
            if args.show_track:
                ...
                img = show_track_img(src_giga_img, temp_track_results,frame_id,len(dataloader))
                cv2.imshow('tracking', img)
                print('2222222222222222222222222222222aaaa')
                cv2.moveWindow('tracking', 20, 20)
                print('333333333aaaaaaaaaaaaaaaaaaaaa')
                show_time = 1
                key = cv2.waitKey(show_time)
                print('333333333aaaaaaaaaaaaaaaaaaaaa44444444444444444444')
        ...
        if args.save_result:
            temp_save_txt_det = '{}/{}'.format(args.output_dir_txt_det,'detection', path.split('/')[-2])
            ...
            temp_save_txt_track = '{}/{}'.format(args.output_dir_txt_det,'tracking', path.split('/')[-2])
            os.makedirs(temp_save_txt_track, exist_ok=True)
            res_file_track = osp.join(temp_save_txt_track, path.split('/')[-1].replace('.jpg', '.txt'))
            np.savetxt(res_file_track, temp_track_results, delimiter=',', fmt='%d')

        frame_id = frame_id + 1
        logger.info(f"save detection results to {res_file}")
        logger.info(f"save tracking results to {res_file_track}")
    temp_save_txt_track = '{}/{}'.format(args.output_dir_txt_det, 'results', path.split('/')[-2])
    res_file = osp.join(temp_save_txt_track, file_name)
